...
public class CVN5  implements Cryptogram
{
	String Pan;
	String PanSeq;
	...
	public byte[] getMAC()
	{
		...
		byte[] R = dfu.hexToBCD( dfu.asciiToHex( dfu.getPadString( '0', 4 ).substring( ApplicationTransactionCounter.length() ).concat( ApplicationTransactionCounter ) + "000000000000" ), 16 );
		...
		//CardVerificationResults use without change
		...
		//byte[] CDOL = new byte[ CDOLData.length() + ( 8 - CDOLData.length() % 8 ) ];
		//CDOL = dfu.hexToBCD( dfu.asciiToHex( CDOLData ), CDOLData.length() );

		//if( CDOLData.length() % 8 != 0 )
		//	CDOL[ CDOLData.length() ] = (byte)0x80;
		...
		int Used = o_utilities.getMessageSize();
